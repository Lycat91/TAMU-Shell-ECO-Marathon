...
void writePWM(uint motorState, uint duty, bool synchronous)
{
    ...
    100%, clamp it to 100%.
    */
    //******** too high of switching will mess with the capacitor so just output a dc voltage aka 255 duty cycle 
    if(duty > 245)
        duty = 255;
    ...
    //******** complement controls the complement mos to the active pwm mosfet. This definition creates a deadband to protect against shorts created by having the high and low side Mosfets on at the same time.
    //******** If not synchronous we dont need this because the low side mos wont be activated
    //******** synchronous is more efficient because the mosfet conducts rather than the body diodes
...
uint get_halls() {
    ...
    //*******Helps with hall sensor noise by taking the most frequent reading rather than just 1. Oversample is declared in the begging and can be changed to take less time or be more precise.
    {
    ...
    //********hall raw is a 3 bit number where each of the bits is contributed by each sensor. EX: sensor1=1 sensor2=0 sensor3=1 => hall_raw = 0b101 => 5
}
